...
def compute_cost_result(learned_model_with_traces, tot_traces):
    ...
    learnedModel_path = "../results/engine/BeforeAnnotation/testData_Output/"  # running from build folder
    testFolder_path = "../examples/engine/TestSet/"
    csv_output_path = "../results/engine/BeforeAnnotation/"


    # ****** writing results to a .csv file  ******
    outFile = csv_output_path + "engine" + "_traces_" + str(learned_model_with_traces) + ".csv"
    # if os.path.exists(outFile):
    #     os.remove(outFile)
    outputFile = open(outFile, "a")
    tableHeader = "Traces" + "\t" + "DTW_distance_speed" + "\n"
    ...
    for i in range(0, tot_traces): #  Testing traces
        ...
        file1 = testFolder_path + "engine_" + str(i+1) + ".csv" # Read the testdata folder containing test traces obtained by simulating the original model
        file2 = learnedModel_path + "engine_" + str(learned_model_with_traces) + "_Trace_" + str(i+1) + ".txt"  # Read the testData_Output folder containing the traces obtained by simulating the learned model
        ...
        inputVar_size = 2 # size of input variable   # int(sys.argv[3])
        outputVar_size = 1 # size of output variable  # int(sys.argv[4])
        # ************************************
        ...
        tableData = str(i + 1) + "\t" + str(dist1) + "\n"
        outputFile.write(tableData)

    outputFile.close()
    benchmark_avg_output1 = total_output1 / tot_traces
    benchmark_std_output1 = (sum([(x - benchmark_avg_output1) ** 2 for x in values_output1]) / (
                tot_traces - 1)) ** 0.5  # (tot_traces -1) is the formula used in LibreOffice's =STDEV

    return benchmark_min_output1, benchmark_max_output1, benchmark_avg_output1, benchmark_std_output1
