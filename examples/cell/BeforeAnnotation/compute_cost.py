...
def compute_cost_result(learned_model_with_traces, tot_traces):
    ...
    learnedModel_path = "../results/cell/BeforeAnnotation/testData_Output/"  # running from build folder
    testFolder_path = "../examples/cell/TestSet/"
    csv_output_path = "../results/cell/BeforeAnnotation/"

    # ****** writing results to a .csv file  ******
    outFile = csv_output_path + "cell" + "_traces_" + str(learned_model_with_traces) + ".csv"
    # if os.path.exists(outFile):
    #     os.remove(outFile)
    outputFile = open(outFile, "a")
    tableHeader = "Traces" + "\t" + "DTW_distance_x" + "\n"
    ...
    for i in range(0, tot_traces):  #  Testing traces
        ...
        file1 = testFolder_path + "excitableCells_" + str(i+1) + ".csv" # Read the testdata folder containing test traces obtained by simulating the original model
        file2 = learnedModel_path + "excitableCells_" + str(learned_model_with_traces) + "_Trace_" + str(i+1) + ".txt"  # Read the testData_Output folder containing the traces obtained by simulating the learned model
